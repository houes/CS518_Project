...
public:
  ...
  public:
	...
	Node(KEY key_, VALUE value_) :key(key_), value(value_), priority(-1), 
		parent(nullptr), left(NULL), right(NULL){}
...
template <class KEY, class VALUE>
typename Treap<KEY, VALUE>::Node* Treap<KEY, VALUE>::lookupGreater_(KEY key) const
{
...
// Set the given key to have the given value.
template <class KEY, class VALUE>
void typename Treap<KEY, VALUE>::insert(typename Treap<KEY, VALUE>::Node * n, KEY key, VALUE value, unsigned int priority)
{
...
// Set the given key to have the given value.
template <class KEY, class VALUE>
void typename Treap<KEY, VALUE>::insert(typename Treap<KEY, VALUE>::Node * n)
{
	KEY k = n->getKey();
	VALUE v = n->getValue();
